...
8. [Generating Test Data](#generating-test-data)
...
1. **Flow Log Format**:
   ...
   - Reference: [AWS VPC Flow Logs Documentation](https://docs.aws.amazon.com/vpc/latest/userguide/flow-log-records.html).
...
│ ├── FlowLogParser.java # Parses flow logs and generates outputs
│ └── LookupTable.java # Loads and queries the lookup table
├── input/
│ ├── flow_logs.txt # Sample flow log file
│ └── lookup_table.csv # Sample lookup table
├── output/
│ ├── tag_counts.csv # Generated tag counts
│ └── port_protocol_counts.csv # Generated port/protocol counts
...
2. **Multiple Tags for Same Port/Protocol**:
...
Run the Program
1. Run the program with input and output directories:
